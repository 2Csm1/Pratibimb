...
def comparison():
    try:
        ...
        for name in AVAILABLE_MODELS:
            ...
            X_test_proc  = pd.DataFrame(pipeline['imputer'].transform(X_test),  columns=PREDICTION_FEATURES)
            X_test_proc  = pd.DataFrame(pipeline['scaler'].transform(X_test_proc),  columns=PREDICTION_FEATURES)
            X_train_proc = pd.DataFrame(pipeline['imputer'].transform(X_train), columns=PREDICTION_FEATURES)
            X_train_proc = pd.DataFrame(pipeline['scaler'].transform(X_train_proc), columns=PREDICTION_FEATURES)

            t0 = time.time()
            pipeline['model'].fit(X_train_proc, y_train)
            training_time = time.time() - t0

            y_pred = pipeline['model'].predict(X_test_proc)
            model_results[name] = {
        ...
        logger.error(f"Error in comparison page: {e}")
        flash(f"Error in comparison page: {e}", "danger")
